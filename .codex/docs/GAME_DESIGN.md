...
Phase 7のMVPでは、Exploreの所要時間は1時間とする。

お出かけ開始時にenergyを10消費する。
...
Phase 7では、外出中の `feed` / `play` / 再度の `go` を禁止する。
...
Phase 7以降、`expedition_count` は `go` でお出かけを開始した回数として扱う。
